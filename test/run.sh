...
go test github.com/cea-hpc/gssapi
...
run_image "service" \
        ...
        "--detach \
        $DOCKER_KDC_OPTS \
        --volume $TEST_DIR/gssapi:/opt/go/src/github.com/cea-hpc/gssapi" >/dev/null
...
if [[ "$OSTYPE" != "darwin"* || "$CLIENT_IN_CONTAINER" != "" ]]; then
        ...
        run_image "client" \
                ...
                $DOCKER_KDC_OPTS \
                --volume $TEST_DIR/gssapi:/opt/go/src/github.com/cea-hpc/gssapi"
else
